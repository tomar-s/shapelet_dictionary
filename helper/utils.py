...
def learn_shapelet_dict_via_sidl(X_batch):
    ...
    for label in np.unique(train_labels):
        ...
        S, A, offsets, F_obj = learn_shapelets_alpha_const(train_data, class_labels, K=5, lambdas=0.1, r=0.5)
...
def data_new_classes(df):

    df = df.reset_index(drop=True)
    class_order = ['walk_sidewalk', 'jog_treadmill', 'downstairs', 'upstairs']  # Adjust class names
    class_data = {label: df[df['label'] == label].reset_index(drop=True) for label in class_order}
    used_indices = {label: 0 for label in class_order}

    epoch_schedule = [5, 4, 5]  # epochs per stage
    class_schedule = [
        class_order[:2],  # initial classes
        class_order[:3],  # add downstairs to initial classes
        class_order  # add upstairs to the three classes already present
    ]
    batch_size = 300
    batches = []

    for stage_epochs, stage_classes in zip(epoch_schedule, class_schedule):
        ...
        for _ in range(stage_epochs):
            ...
            for i, cls in enumerate(stage_classes):
                ...
                class_df = class_data[cls].iloc[start:end]
                used_indices[cls] = end
                epoch_data.append(class_df)

            batch_df = pd.concat(epoch_data).reset_index(drop=True)
...
def normalise_batch(x):
    ...
    mean_batch = x.mean(axis=1, keepdims=True)
    std_batch = x.std(axis=1, keepdims=True)
    std_batch[std_batch == 0] = 1.0
    X_batch_norm = (x - mean_batch) / std_batch
