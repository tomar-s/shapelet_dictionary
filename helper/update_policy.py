...
def euclidean_distance(sequence_s, sequence_r):
    ...
    if len(s) != len(r):
        raise ValueError("Sequences should be same length")
...
def find_closest_shapelets(shapelets_a, shapelets_b, top_k=1):
    ...
    # for each shapelet in A, find the top_k closest shapelets from B.
    # in a dictionary mapping each shapelet index in A to a list of (distance, shapelet_index) tuples
...
def combine_random_shapelets(shapelets_a, shapelets_b):
   ...
   # get 10 random shapelets from B (from 20) and combine them to set A to get combined set of 20
    shapelets_a = np.array(shapelets_a)
    shapelets_b = np.array(shapelets_b)
...
def combine_topK_previous(shapelet_a, shapelet_b, overall_ranks):
    ...
    # get top k shapelets from B which are ranked based on sparse coefficient matrix and combine them to shapelets A.
    # combined set of 20
